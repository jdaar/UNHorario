    ...
    <ScrollArea class="border rounded-md h-full whitespace-nowrap w-full p-md">
      <ul class="w-full h-full relative">
        <li
          ...
          v-bind:key="day"
          class="w-full h-full flex content-center"
        >
          ...
          <div>
            <div class="w-96 h-full flex flex-row">
              <div
                ...
                v-bind:key="segment"
                class="h-full flex justify-start content-start flex-col flex-wrap min-h-20 min-w-20 border"
              >
                <div class="p-1">
                  <Label
                ...
                <div class="gap-1 flex flex-col w-full p-1">
                  <div
                    v-for="lecture in lecturesByDay[parseInt(day) + 1]"
                    v-bind:key="`${lecture.course_name}-${day}-${segment}`"
                    class="w-full h-full"
                  >
                    <div
                      :style="`background-color: ${lecture.color}`"
                      class="w-full rounded border max-h-20"
                      v-if="isDateBetween(lecture, segment)"
                    >
                      <Label class="text-xs p-1 text-muted"
                        >{{ lecture.course_name.slice(0, 9) }}...</Label
                      >
                    </div>
                  </div>
                </div>
