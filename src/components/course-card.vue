...
<template>
  <ContextMenu>
      ...
      <div class="gap-md flex" v-if="course != null">
        <div class="w-1/6 flex content-center flex-wrap justify-center">
          <div
            ...
            class="w-12 p-0 text-center justify-center align-center content-center flex flex-wrap rounded-full mt-1 mb-2"
          >
            <svg
              width="32"
              height="28"
          ...
          <Label class="text-center"
            ><span :style="`color: ${course.color}`">{{
              selectedCourseGroup
                ? courseGroupsById[selectedCourseGroup][0].quotas
                : 0
            }}</span>
            cupos</Label
          >
        ...
        <div class="w-full">
          ...
          <ul>
            ...
            <li class="flex content-center flex-wrap gap-2">
              <div
                class="flex content-center flex-col justify-center flex-wrap"
              >
                <svg
                  width="16"
                  height="14"
              ...
              <p class="text-sm text-muted-foreground w-2/3">
                {{
                  selectedCourseGroup
                    ? courseGroupsById[selectedCourseGroup][0].teachers
                    : "Seleccione un grupo"
                }}
              </p>
            </li>
          ...
          <Select v-model="selectedCourseGroup">
            ...
            <SelectContent>
              <ScrollArea class="h-32">
                <SelectGroup
                  v-bind:key="teacher"
                  v-for="teacher in Object.keys(courseGroupsByTeacher)"
                >
                  <SelectLabel>{{ teacher }}</SelectLabel>
                  <SelectItem
                    v-bind:key="`${group.group_id}`"
                    v-bind:value="`${group.group_id}`"
                    v-for="group in courseGroupsByTeacher[teacher]"
                  >
                    {{ group.group_name }}
                  </SelectItem>
                </SelectGroup>
    ...
    <ContextMenuContent class="w-64">
      ...
      <ContextMenuItem inset disabled> Organizar </ContextMenuItem>
    </ContextMenuContent>
  </ContextMenu>
</template>
