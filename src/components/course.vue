            ...
            <details role="list">
                <summary>{{
                    course.selectedGroup ? course.groups.filter(v => v.number ==
                        course.selectedGroup)[0].name : 'Seleccione un grupo'
                }}</summary>
                <ul role="listbox" class="group-container">
                    <li v-for="group in course.groups.map((e) => ({ number: e.number, name: e.name }))"
                        v-on:click="courses.selectGroup(course.code, group.number)" v-bind:key="group.number">
                        <p>
                            Grupo {{ group.name }} ({{
